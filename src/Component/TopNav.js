...
const TopNav = () => {
    ...
    return (
        <div className='max-w-[1520px] mx-auto flex justify-between items-center p-4'>
            ...
            <div className='bg-gray-200 rounded-full flex items-center px-2 w-[] sm:w-[400px] lg:w-[500px]'>
                <AiOutlineSearch size={25}/>
                <input className='bg-transparent focus:outline-none p-2 w-full ' type='text' placeholder='Search for cuisines,meals...'/>
            </div>
            <button className='bg-orange-700 text-white hidden md:flex rounded-full py-2 items-center'>
                <BsFillCartFill size={20}/>Cart
                </button>
                {
                    sideNav? 
                    (<div className='bg-black/60 fixed w-full h-screen z-10 top-0 left-0' onClick={()=>setSideNav(!sideNav)}></div>) : null
                }
                <div className={ sideNav? 'fixed top-0 left-0 w-[300px] h-screen bg-white z-10 duration-300 ' :
                    ...
                    <nav>
                        <ul className='flex flex-col p-4 text-grey-900'>
                            <li className='text-xl py-4 flex'> <BsPerson size={25} className='mr-4 text-white bg-black rounded-full'/> My Accout</li>
